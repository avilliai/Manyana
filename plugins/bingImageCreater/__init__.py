        ...
        args = {
            'linux_cookies': _genarate_nix_paths_chromium(
                [
                    ...
                    '~/.var/app/com.yandex.browser/config/yandex-browser{channel}/Default/Cookies',
                    '~/.var/app/com.yandex.browser/config/yandex-browser{channel}/Profile */Cookies'
                ],
                channel=['', '-beta', '-unstable']
            ),
            'windows_cookies': _genarate_win_paths_chromium(
            ...
            'osx_cookies': _genarate_nix_paths_chromium(
                [
                    '~/Library/Application Support/Yandex/Browser{channel}/Default/Cookies',
                    '~/Library/Application Support/Yandex/Browser{channel}/Profile */Cookies'
                ],
                channel=['', ' Beta', ' Dev']
            ),
            'windows_keys': _genarate_win_paths_chromium(
            ...
            'os_crypt_name': 'yandex-browser',
            'osx_key_service': 'Yandex Safe Storage',
            'osx_key_user': 'Yandex'
        }
        super().__init__(browser='Yandex Browser', cookie_file=cookie_file,
                         domain_name=domain_name, key_file=key_file, **args)
...
def yandex(cookie_file=None, domain_name="", key_file=None):
    """Returns a cookiejar of the cookies used by Yandex. Optionally pass in a
    domain name to only load cookies from the specified domain
    """
...
def load(domain_name=""):
    ...
    """
    cj = http.cookiejar.CookieJar()
    for cookie_fn in [chrome, chromium, yandex, opera, opera_gx, brave, edge, vivaldi, firefox, librewolf, safari]:
        try:

...
def create_chinese_wordcloud(text, save_path="./test.png"):
    """
    生成指定形状和字体颜色的中文词云图，并自动调整字体大小以适应 PNG 图像范围。
    如果词总数少于100，则重复单词直到达到100。
    """
...
async def appendData(text,groupid,userid):
    """
    异步生成词云图
    """
    loop = asyncio.get_running_loop()
...
def record_user_input(text,groupid,userid):
...
async def getMyAllText(group_path, user_id):
    loop = asyncio.get_running_loop()
    with ProcessPoolExecutor() as pool:
        # 使用 await 等待 run_in_executor 完成，并将结果赋给 result 变量
        result = await loop.run_in_executor(
            pool,
            find_and_read_user_files,  # 您的文件读取函数
            group_path,
            user_id
        )
    return result

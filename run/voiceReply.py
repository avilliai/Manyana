...
def main(bot,master,app_id,app_key,logger):
    ...
    async def botSaid(event:GroupMessage):
        ...
        if m and str(event.message_chain).split("说")[0] not in characters and len(str(event.message_chain))<50:
            # 取出指令中的地名
            text = m.group(1)
    ...
    async def botSaid(event: GroupMessage):
        ...
        if m and str(event.message_chain).split("中文")[0] not in characters and len(str(event.message_chain))<50:
            # 取出指令中的地名
            text = m.group(1)
            path = '../data/voices/' + random_str() + '.wav'
            #text = await translate(text, app_id, app_key)
    ...
    async def botSaid(event: GroupMessage):
        ...
        if m and str(event.message_chain).split("日文")[0] not in characters and len(str(event.message_chain))<50:
            # 取出指令中的地名
            text = m.group(1)
            path = '../data/voices/' + random_str() + '.wav'
            # text = await translate(text, app_id, app_key)

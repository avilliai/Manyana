    ...
    async def youtube_download(event: GroupMessage):
        if str(event.message_chain).startswith('youtube视频下载'):
            try:
                ...
                video_url = await get_video(video_id)
            ...
            try:
                ...
                audio_url = await get_audio(video_id)
            ...
            try:
                ...
                img_url = await get_img(video_id)
                await bot.send(event, [f"{video_id}的封面：", Image(url=img_url)], True)
            except:

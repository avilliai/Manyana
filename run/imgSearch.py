        ...
        if "搜图" in str(event.message_chain) and event.message_chain.count(Image):
            if event.group.id not in dataGet:
                ...
                try:
                    ...
                    dataGet.get(event.group.id).append(b1)
                ...
                try:
                    ...
                    # b1 = ForwardMessageNode(sender_id=bot.qq, sender_name="Manyana",message_chain=MessageChain([result, Image(url=urlss]))
                    dataGet.get(event.group.id).append(b1)
                await bot.send(event,Forward(node_list=dataGet.get(event.group.id)))
                dataGet.pop(event.group.id)
            else:
                await bot.send(event,"当前服务器正忙，请稍后再试")

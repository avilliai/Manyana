...
def main(bot, master, botName, logger):
    ...
    async def music2(event: FriendMessage):
        ...
        try:
            ...
            logger.info(sender)
        if event.message_chain.count(MusicShare) and (str(event.sender.id) in sender.keys()) and sender.get(
                str(event.sender.id)).get("step") == 1:
            ...
            try:
                logger.info("点歌数据构建完成：\n日期:" + str(today) + "\n留言：" + sender.get(str(event.sender.id)).get(
                    "message"))

                await bot.send_friend_message(int(master),
                                              "点歌数据构建完成：\n日期:" + str(today) + "\n留言：" + sender.get(
                                                  str(event.sender.id)).get("message") + "点歌人：" + str(
                                                  event.sender.nickname) + str(event.sender.id))
            except:
        ...
        for i in severGroups:
            ...
            try:
                ...
                await bot.send_group_message(intTrans, MusicShare(kind=music.get("kind"), title=music.get("title"),
                                                                  ...
                                                                  picture_url=music.get("picture_url"),

...
def main(bot,logger):
    ...
    randomASMRList=os.listdir("data/ASMR/randomASMR")
    ...
    async def moyuToday(event: GroupMessage):
        if ("喜加一" in str(event.message_chain) and At(bot.qq) in event.message_chain) or str(
            ...
            try:
                ...
                logger.info("获取喜加一结果")
                await bot.send(event, (path,Image(path="data\pictures\others\steamadd1.jpg")), True)
            except:
                logger.error("衮")
                await bot.send(event, "获取喜加一结果失败，请稍后再试")

    @bot.on(GroupMessage)
    async def sleepassistant(event: GroupMessage):
        if ("提醒睡觉小助手" in str(event.message_chain) and At(bot.qq) in event.message_chain) or str(
                event.message_chain) == "提醒睡觉小助手":
            logger.info("获取提醒睡觉小助手")
            logger.info("推送提醒睡觉小助手")
            path="data\pictures\others\sleepassistant.png"
            try:
                await bot.send(event, [data.get("sleepassistant").get("text"),Image(path=path) ], True)
            except:
                logger.error("衮")
                await bot.send(event, "推送提醒睡觉小助手失败，请稍后再试")
    @bot.on(GroupMessage)
    ...
    @bot.on(GroupMessage)
    async def hitokoto(event: GroupMessage):
        
        if ("动漫一言" in str(event.message_chain) and At(bot.qq) in event.message_chain) or str(event.message_chain) == "动漫一言":
            logger.info("获取动漫一言")
            url = 'https://v1.hitokoto.cn/?c=a&c=b'
        elif ("游戏一言" in str(event.message_chain) and At(bot.qq) in event.message_chain) or str(event.message_chain) == "游戏一言":
            logger.info("获取游戏一言")
            url = 'https://v1.hitokoto.cn/?c=c'
        elif ("文学一言" in str(event.message_chain) and At(bot.qq) in event.message_chain) or str(event.message_chain) == "文学一言":
            logger.info("获取文学一言")
            url = 'https://v1.hitokoto.cn/?c=d'
        elif ("哲学一言" in str(event.message_chain) and At(bot.qq) in event.message_chain) or str(event.message_chain) == "哲学一言":
            logger.info("获取哲学一言")
            url = 'https://v1.hitokoto.cn/?c=k'
        elif ("网络一言" in str(event.message_chain) and At(bot.qq) in event.message_chain) or str(event.message_chain) == "网络一言":
            logger.info("获取网络一言")
            url = 'https://v1.hitokoto.cn/?c=f'
        elif ("一言" in str(event.message_chain) and At(bot.qq) in event.message_chain) or str(event.message_chain) == "一言":
            logger.info("获取一言")
        ...
        try:
            async with httpx.AsyncClient(timeout=20) as client:
                r = await client.get(url)
                logger.info(r.json().get("hitokoto","from"))
                try:
            ...
            logger.error(e)
            await bot.send(event, "获取一言失败，请稍后再试")
    @bot.on(GroupMessage)    
    async def lolimi(event: GroupMessage):
        if str(event.message_chain).startswith("二次元少女") or str(event.message_chain).startswith("穿越到二次元"):
            try:
                name=event.sender.member_name
                if At in event.message_chain:
                    id=event.message_chain.split("at:")[1].split("]")[0]
                    name=id
                url = 'https://api.lolimi.cn/API/Ser/?name='+ name
                async with httpx.AsyncClient(timeout=20) as client:
                    r = await client.get(url)
